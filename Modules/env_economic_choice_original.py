...
class EconomicChoiceEnv(gym.Env):
    ...
    def _select_trial_conditions(self):
        ...
        delay_ms = self.rng.uniform(self._durations_ms['Offer_min'],
                                    self._durations_ms['Offer_max'])
    ...
    def _get_observation(self, current_epoch):
        ...
        # Active during Offer and Decision so the agent sees the offer
        if current_epoch in [EPOCH_OFFER, EPOCH_DECISION]:
            juiceL, juiceR = self.trial_juice_LR
